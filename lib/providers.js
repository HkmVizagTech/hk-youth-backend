...
// Set global options BEFORE models are heavily used
mongoose.set('bufferTimeoutMS', 30000);

console.log("🛠️  Initializing Mongoose connection...");

mongoose.connect(MONGODB_URI, {
    serverSelectionTimeoutMS: 15000,
    socketTimeoutMS: 45000,
    maxPoolSize: 10,
    family: 4 // Use IPv4 to avoid dns issues on some systems
})
    .then(() => {
        const maskedUri = MONGODB_URI ? MONGODB_URI.replace(/:([^@]+)@/, ":****@").split('?')[0] : "unknown";
        console.log(`✅ Connected to MongoDB via Mongoose (${maskedUri})`);
    })
    .catch(err => {
